...
function start_carpenter( origin )
{
	...
	for(i = 0; i < players.size; i++)
	{
		...
		// refill shield health
		if (players[i] HasWeapon(level.weaponRiotshield)) 
		{
			// get current shield health
			shield_health = players[i] DamageRiotShield(0);
			shield_health_to_max = level.weaponRiotshield.weaponstarthitpoints - shield_health;

			// negative dmg to shield
			players[i] DamageRiotShield(-shield_health_to_max);
			players[i] riotshield::player_set_shield_health(1500, 1500);
		}
	}
