...
/#	PrintLn( "ZM >> round_think start" );	#/
	...
	for( ;; )
	{
		...
		else 
		{
			...
			{
				players = GetPlayers();
				player_has_afterlife = 0;
				alNum = RandomIntRange( 1, 6 );
				normNum = RandomIntRange( 1, 4 );
				// First check if ANY player is in afterlife
				foreach ( player in GetPlayers() )
				{
					if ( player.afterlife )
						player_has_afterlife = 1;
				}
				foreach ( player in GetPlayers() ) 
				{
					// Don't play both sounds if splitscreen game and a player is in afterlife..
					if ( players.size > 1 && level.is_forever_solo_game && player_has_afterlife )
					{
						player PlaySoundToPlayer( "mus_alroundstart" + alNum + "_intro", player );
						//level thread zm_audio::sndMusicSystem_PlayState( "round_start_afterlife" );
					}
					else if ( player.afterlife ) 
					{
						player PlaySoundToPlayer( "mus_alroundstart" + alNum + "_intro", player );
					} 
					else 
					{
						if ( level.round_number == 1 )
							player PlaySoundToPlayer( "mus_roundstart_first_intro", player );
						else
							player PlaySoundToPlayer( "mus_roundstart" + normNum + "_intro", player );
					}
				}
...
#/
	...
	while( 1 )
	{
		...
		if( !should_wait )
		{
			foreach (player in GetPlayers())
			{
				player PlaySoundToPlayer( "mus_roundend1_intro", player );
			}
