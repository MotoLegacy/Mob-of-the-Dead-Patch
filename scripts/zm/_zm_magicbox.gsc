...
#precache( "fx", "custom/jerri/al_box/al_box_ray" );
...
function boxstub_update_prompt( player )
{
	...
	else
	{
		...
		{
			self.hint_string = zm_utility::get_hint_string( self, "locked_magic_box_cost" );
		}
		else
		{
...
function default_pandora_fx_func( )
{
	...
	self.pandora_light.angles = self.zbarrier.angles + (0, 0, 0);
...
function treasure_chest_think()
{
	...
	while( 1 )
	{
		...
		if ( isdefined( level.using_locked_magicbox ) && level.using_locked_magicbox && isDefined( self.is_locked ) && self.is_locked )
		{
			...
			{
				user zm_score::minus_to_player_score( level.locked_magic_box_cost );
			...
			{
				user zm_audio::create_and_play_dialog( "general", "no_money_box" );
			}
			wait 0.1;
		}
		
		// Make sure the user is a player, and that they can afford it
		else if( IsDefined(self.auto_open) && zm_utility::is_player_valid( user ) )
		{
	...
	if ( isDefined( level.using_locked_magicbox ) && level.using_locked_magicbox )
	{
		self thread zm_magicbox_lock::watch_for_lock();
	}
	self._box_open = true;
...
function treasure_chest_weapon_spawn( chest, player, respin )
{
	if ( isDefined( level.using_locked_magicbox ) && level.using_locked_magicbox )
	{
		...
		self thread zm_magicbox_lock::clean_up_locked_box();
	}
