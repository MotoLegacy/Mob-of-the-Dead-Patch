...
function main()
{
	//level thread give_gun();
	level thread check_map();
...
function give_gun()
{
	...
	IPrintLnBold("screen gone");
	wait 10;
	IPrintLnBold("give weapon");
	minigun_alcatraz = GetWeapon("shotgun_acidgat");
	foreach(player in GetPlayers())
	{
		player GiveWeapon(minigun_alcatraz);
		player SwitchToWeapon(minigun_alcatraz);
	}
}
